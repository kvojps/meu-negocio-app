...
type DashboardPeriod = 'week' | 'month' | 'custom';
...
function getCurrentWeekRange(referenceDate: Date): { start: Date; end: Date } {
...
function getCurrentMonthRange(referenceDate: Date): { start: Date; end: Date } {
  const start = new Date(referenceDate.getFullYear(), referenceDate.getMonth(), 1);
  const end = new Date(referenceDate.getFullYear(), referenceDate.getMonth() + 1, 0);
  return { start: startOfLocalDay(start), end: endOfLocalDay(end) };
}

function formatDashboardDateLabel(date: Date): string {
...
function getDashboardRange(
  ...
  const start = customStartDate ? startOfLocalDay(new Date(`${customStartDate}T00:00:00`)) : startOfLocalDay(new Date(now.getFullYear(), now.getMonth(), 1));
  const end = customEndDate ? endOfLocalDay(new Date(`${customEndDate}T00:00:00`)) : endOfLocalDay(now);
...
function isSaleWithinRange(sale: Sale, start: Date, end: Date): boolean {
  const saleDate = new Date(sale.date);
  return saleDate >= start && saleDate <= end;
}

function buildDashboardBuckets(sales: Sale[], start: Date, end: Date): DashboardBucket[] {
...
function formatDashboardAxisLabel(date: Date): string {
  return date.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' });
}
    ...
    <div className={`app-shell ${sidebarOpen ? 'sidebar-open' : 'sidebar-collapsed'}`}>
      ...
      <main className="content">
        {activeSection === 'dashboard' ? (
          <DashboardSection sales={sales} loading={loadingSales} onOpenSale={openSaleDetails} />
