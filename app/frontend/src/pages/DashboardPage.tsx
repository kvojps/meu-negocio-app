...
function IconCount() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      <path
        fillRule="evenodd"
        d="M3 4a1 1 0 0 1 1-1h12a1 1 0 0 1 0 2H4a1 1 0 0 1-1-1Zm0 4a1 1 0 0 1 1-1h12a1 1 0 0 1 0 2H4a1 1 0 0 1-1-1Zm0 4a1 1 0 0 1 1-1h12a1 1 0 0 1 0 2H4a1 1 0 0 1-1-1Zm0 4a1 1 0 0 1 1-1h6a1 1 0 0 1 0 2H4a1 1 0 0 1-1-1Z"
        clipRule="evenodd"
      />
    </svg>
  );
...
function IconCost() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      <path
        fillRule="evenodd"
        d="M2 10a8 8 0 1 1 16 0 8 8 0 0 1-16 0Zm5-2.25A.75.75 0 0 1 7.75 7h4.5a.75.75 0 0 1 0 1.5h-4.5A.75.75 0 0 1 7 7.75Zm0 2.5a.75.75 0 0 1 .75-.75h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1-.75-.75Zm.75 2a.75.75 0 0 0 0 1.5h2a.75.75 0 0 0 0-1.5h-2Z"
        clipRule="evenodd"
      />
    </svg>
  );
...
function IconProfit() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      <path
        fillRule="evenodd"
        d="M12 7a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm-1 9a1 1 0 0 0 1-1v-4a1 1 0 1 0-2 0v4a1 1 0 0 0 1 1Z"
        clipRule="evenodd"
      />
      <path
        fillRule="evenodd"
...
export function DashboardPage({
  ...
  const [customStartDate, setCustomStartDate] = useState(
    toDateInputValue(currentMonthRange.start),
  );
  const [customEndDate, setCustomEndDate] = useState(
    toDateInputValue(currentMonthRange.end),
  );
  ...
  const chartMinValue = Math.min(
    0,
    ...buckets.map((bucket) => Math.min(bucket.revenue, bucket.profit)),
  );
  const chartMaxValue = Math.max(
    0,
    ...buckets.map((bucket) => Math.max(bucket.revenue, bucket.profit)),
  );
  const chartValueRange = Math.max(chartMaxValue - chartMinValue, 1);
  const chartPoints = buckets.map((bucket, index) => {
    const x =
      buckets.length > 1
        ? chartPaddingX + (index / (buckets.length - 1)) * chartInnerWidth
        : chartPaddingX + chartInnerWidth / 2;
    const revenueY =
      chartPaddingY +
      chartInnerHeight -
      ((bucket.revenue - chartMinValue) / chartValueRange) * chartInnerHeight;
    const profitY =
      chartPaddingY +
      chartInnerHeight -
      ((bucket.profit - chartMinValue) / chartValueRange) * chartInnerHeight;
    ...
    return {
      ...
      revenueY,
      profitY,
      displayLabel: formatDashboardAxisLabel(
        new Date(`${bucket.key}T12:00:00`),
      ),
    };
  });
  const revenueLinePath = chartPoints
    .map(
      (point, index) =>
        `${index === 0 ? "M" : "L"} ${point.x.toFixed(2)} ${point.revenueY.toFixed(2)}`,
    )
    .join(" ");
  const profitLinePath = chartPoints
    .map(
      (point, index) =>
        `${index === 0 ? "M" : "L"} ${point.x.toFixed(2)} ${point.profitY.toFixed(2)}`,
    )
    .join(" ");
    ...
    <section className="dashboard-stack">
      <section className="table-card dashboard-card">
        <div className="table-card-header dashboard-header">
          <div>
            ...
            <p>
              {loading
                ? "Carregando..."
                : `${filteredSales.length} receita(s) no período selecionado`}
            </p>
          </div>

          <div
            className="dashboard-period-controls"
            role="group"
            aria-label="Selecionar período do dashboard"
          >
            <button
              className={`ghost-button ${period === "week" ? "active-control" : ""}`}
              type="button"
            ...
            <button
              className={`ghost-button ${period === "month" ? "active-control" : ""}`}
              type="button"
            ...
            <button
              className={`ghost-button ${period === "custom" ? "active-control" : ""}`}
              type="button"
        ...
        <p className="dashboard-range-label">
          {range.label}: {formatRangeLabel(range.start, range.end)}
        </p>
        ...
        <div className="dashboard-chart-wrap">
          <div className="dashboard-chart-legend">
            <span>
              <span className="legend-swatch revenue-swatch" />
              Faturamento diário
            </span>
            <span>
              <span className="legend-swatch profit-swatch" />
              Lucro bruto
            </span>
          </div>
          ...
          <div
            ...
            aria-label="Receitas agregadas por dia"
          >
            {chartPoints.length === 0 ? (
              ...
              <svg
                ...
                role="img"
                aria-label="Gráfico de linha de receitas por dia"
              >
                <line
                  x1={chartPaddingX}
                  y1={chartPaddingY + chartInnerHeight}
                  x2={chartPaddingX + chartInnerWidth}
                  y2={chartPaddingY + chartInnerHeight}
                ...
                <line
                  ...
                  x2={chartPaddingX}
                  y2={chartPaddingY + chartInnerHeight}
                  className="dashboard-axis dashboard-axis-vertical"
                />
                {peakRevenue > 0
                  ? [0, 1, 2, 3].map((tick) => {
                      const tickValue =
                        chartMinValue + (chartValueRange / 3) * tick;
                      const y =
                        chartPaddingY +
                        chartInnerHeight -
                        (tick / 3) * chartInnerHeight;
                ...
                <path
                  d={revenueLinePath}
                  className="dashboard-line revenue-line"
                />
                <path
                  d={profitLinePath}
                  className="dashboard-line profit-line"
                />
                {chartPoints.map((point) => (
                  <g key={point.key}>
                    <circle
                      ...
                      cy={point.revenueY}
                      r="5"
                      className="dashboard-point revenue-point"
                    >
                    ...
                    <circle
                      ...
                      cy={point.profitY}
                      r="5"
                      className="dashboard-point profit-point"
                    >
        ...
        <div className="table-card-header">
          <div>
            ...
            <p>
              {loading
                ? "Carregando..."
                : `${recentSales.length} item(ns) exibido(s)`}
            </p>
          </div>
        </div>
