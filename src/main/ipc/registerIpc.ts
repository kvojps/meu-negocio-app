...
export function registerIpcHandlers(db: Database.Database): void {
  ...
  ipcMain.handle(
    ...
    (_event, id: unknown, newStatus: unknown) =>
      setOrderStatus(
        db,
        parseId(id),
        parseOrThrow(orderStatusSchema, newStatus),
      ),
  );
  ipcMain.handle(
    IPC_CHANNELS.ordersSetPaymentAmount,
    (_event, id: unknown, amountPaid: unknown) =>
      setOrderPaymentAmount(
        db,
        parseId(id),
  ...
  ipcMain.handle(IPC_CHANNELS.ordersDelete, (_event, id: unknown) =>
    deleteOrder(db, parseId(id)),
  );
